...
export class TasksPage{
    ...
    async toggle(taskName: string){
      const target =  await this.page.locator(`//p[text()="${taskName}"]/..//button[contains(@class, "Toggle")]`)
      await target.click()
    }
    async removeTask(taskName: string){
        const target =  await this.page.locator(`//p[text()="${taskName}"]/..//button[contains(@class, "Delete")]`)
        await target.click()
      }
